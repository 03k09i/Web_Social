...
import { addShortCutAction, getShortCutAction, deleteShortCutAction } from '../../store/actions/shortcut.actions'
  ...
  return (
    ...
    >
      ...
      <ul className="menu menu2">
        <p style={{ fontWeight: "bold", padding: "0 16px", margin: "10px 10px 10px 0" }}>Truy cập nhanh</p>
        {
          listShortcut.map((itemShortcut, index) => (
            <li className="menu-item" key={index} >
              <NavLink className="menu-item-link" to={`/profile/${itemShortcut.shortcut._id}`}>
                <div className="menu-item-link-icon icon-overview">
                  <img
                    className="image-avatar-40" style={{ marginTop: "-30px", marginLeft: "-2px" }}
                    src={
